...
<body>
	...
	차량 이름 검색 : <input type="text" name="car_name"><br> <input type="submit" value="검색">
	</form>
	<form METHOD="post" ACTION="1.search_by_dis_driven.jsp">
	주행거리 검색 : <input type="text" name="dis_driven"><br> <input type="submit" value="검색">
	</form>
	</header>
	<section>
		<%
		...
		<div>
			...
			%>
			<a
				href="<%=request.getContextPath()%>/NewFile.jsp?currentPage=<%=currentPage - 1%>">이전</a>
			<%
			}
			if (currentPage < lastPage) {
			%>
			<a
				href="<%=request.getContextPath()%>/NewFile.jsp?currentPage=<%=currentPage + 1%>">다음</a>
			<%
			}
			%>
		</div>
