...
<body>
	...
	<header>
		...
		<form METHOD="post" ACTION="1.search_by_car_name.jsp">
			차량 이름 : <input type="text" name="car_name"><br>
			<inputtype="submit" value="등록">
			<%	
	...
	<section>
		...
		<div>
			...
			if (currentPage > 1) {
			%>
			<a href="<%=request.getContextPath()%>/1.search_by_car_name.jsp?currentPage=<%=currentPage - 1%>&car_name=<%=car_name%>">이전</a>
			<%
			}
			if (currentPage < lastPage) {
			%>
			<a href="<%=request.getContextPath()%>/1.search_by_car_name.jsp?currentPage=<%=currentPage + 1%>">다음</a>
			<%
			}
			%>
		</div>
