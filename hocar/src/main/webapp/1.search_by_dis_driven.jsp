...
<fmt:requestEncoding value="UTF-8" /> 
	...
	<header>
		...
		<form METHOD="post" ACTION="1.search_by_dis_driven.jsp">
			주행 거리 : <input type="text" name="dis_driven" value=<%=dis_driven%>><br> <input
	...
	<section>
		...
		System.out.println("dis_driven:" + dis_driven);
		...
		try {
			...
			String totalSql = "Select count(*)from usedcar where dis_driven < ?";
			totalpstmt = con.prepareStatement(totalSql);
			totalpstmt.setInt(1, dis_driven);
		...
		<%
		int pagePerRow = 25;
		String listSql = "select * from usedcar where dis_driven < ? order by dis_driven desc limit ?,?";

		listpstmt = con.prepareStatement(listSql);
		listpstmt.setInt(1, dis_driven);
		...
		String Sdis_driven = Integer.toString(dis_driven);
		%>
<c:url value="http://localhost:8080/hocar/1.search_by_dis_driven.jsp" var="next"> 
<c:param name="currentPage" value="<%=SnextPage %>"/> 
<c:param name="dis_driven" value="<%=Sdis_driven %>"/>
</c:url> 
<c:url value="http://localhost:8080/hocar/1.search_by_dis_driven.jsp" var="back"> 
